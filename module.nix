...
let
  ...
  relayMailScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      ${feed-reader.env}/bin/bundle exec rake action_mailbox:ingress:postfix URL='https://${cfg.hostname}/rails/action_mailbox/relay/inbound_emails' INGRESS_PASSWORD="$RAILS_INBOUND_EMAIL_PASSWORD" >$FILENAME 2>&1
      cat $FILENAME
    '';
  };
in
